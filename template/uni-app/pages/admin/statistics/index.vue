<template>
	<div class="statistical-page" ref="container">
		...
		<div class="wrapper">
			<div class="title">
				{{ time == 'date'?'':title }}{{ this.where.type == 1 ? $t(`turnover`) : $t(`order_volume`) }}
			</div>
			<div class="money">{{ time_price }}</div>
			<div class="increase acea-row row-between-wrapper">
				<div>
					{{ time == 'date'?'':title }}{{$t(`growth_rate`)}}：<span :class="increase_time_status === 1 ? 'red' : 'green'">{{ increase_time_status === 1 ? "" : "-" }}{{ growth_rate }}%
              ...
              "></span></span>
				...
				<div>
					{{ time == 'date'?'':title }}{{$t(`increase`)}}：<span :class="increase_time_status === 1 ? 'red' : 'green'">{{ increase_time_status === 1 ? "" : "-" }}{{ increase_time }}
						<span class="iconfont" :class="
                increase_time_status === 1
                  ? 'icon-xiangshang1'
                  : 'icon-xiangxia2'
		...
		<div class="chart">
			<div class="chart-title">
				{{$t(`unit`)}}（{{where.type == 1?$t(`svip_tip2`):$t(`share`)}}）
			</div>
		...
		<div class="public-wrapper">
			<div class="title">
				<span class="iconfont icon-xiangxishuju"></span>{{$t(`detailed_data`)}}
			</div>
			<div class="nav acea-row row-between-wrapper">
				<div class="data">{{$t(`date`)}}</div>
				<div class="browse">{{$t(`number_order`)}}</div>
				<div class="turnover">{{$t(`turnover_yuan`)}}</div>
			</div>
			<div class="conter">
...
<script>
	...
	export default {
		...
		computed: {
			monthRangeText() {
				return this.monthRange.length ? this.$t(`fixed`) : this.$t(`specified_range`);
			}
		},
		methods: {
			getIndex: function() {
