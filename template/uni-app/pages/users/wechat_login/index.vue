	...
	<view :style="colorStyle">
		...
		<view class="wechat_login">
			...
			<view class="btn-wrapper">
				<!-- #ifdef H5 -->
				<button hover-class="none" @click="wechatLogin" class="bg-green btn1">{{$t(`wechat_login`)}}</button>
				<!-- #endif -->
				<!-- #ifdef MP -->
				<button v-if="canUseGetUserProfile && code" hover-class="none" @tap="getUserProfile"
					class="bg-green btn1">{{$t(`wechat_login`)}}</button>
				<button v-else hover-class="none" open-type="getUserInfo" @getuserinfo="setUserInfo"
					class="bg-green btn1">{{$t(`wechat_login`)}}</button>
				<!-- #endif -->
				<button hover-class="none" @click="isUp = true" class="btn2">{{$t(`phone_login`)}}</button>
			</view>
		</view>
		<block v-if="isUp">
