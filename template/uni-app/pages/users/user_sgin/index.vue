	...
	<view :style="colorStyle">
		<view class='sign' :style="colorStyle">
				...
				<view class='headerCon acea-row row-between-wrapper'>
					<view class='left acea-row row-between-wrapper'>
						...
						<view class='text'>
							...
							<view class='integral acea-row'><text>{{$t(`point`)}}: {{userInfo.integral}}</text></view>
						</view>
					</view>
			...
			<view class='wrapper'>
				<view class='list acea-row row-between-wrapper'>
					<view class='item' v-for="(item,index) in signSystemList" :key="index">
						<view :class='(index+1) == signSystemList.length ? "rewardTxt" : ""'>
							{{(item.day==$t(`the_seventh_day`) || item.day==$t(`day_7`))?$t(`award`):item.day}}
						</view>
						<view class='venus'
				...
				<button class='but bg-color on' v-if="userInfo.is_day_sgin">{{$t(`signed_in`)}}</button>
				<form @submit="goSign" v-else>
					<button class='but bg-color' formType="submit">{{$t(`sign_in_now`)}}</button>
				</form>
				<view class='lock'></view>
			</view>
			<view class='wrapper wrapper2'>
				<view class='tip'>{{$t(`accumulated_sign_in`)}}</view>
				<view class='list2 acea-row row-center row-bottom'>
					...
					<view class='data'>{{$t(`days`)}}</view>
				</view>
				<view class='tip2'>{{$t(`said_consecutivw`)}}{{day}}{{$t(`get_excess`)}}</view>
				<view class='list3'>
					...
					<view class='loading' @click='goSignList' v-if="signList.length >= 3">{{$t(`click_load_more`)}}<text
			...
			<view class='signTip acea-row row-center-wrapper' :class='active==true?"on":""'>
				...
				<view class='signTipCon'>
					...
					<view class='state'>{{$t(`sign_in_success`)}}</view>
					<view class='integral'>{{$t(`get`)}}{{integral}}{{$t(`point`)}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			Rp: function(n) {
				let cnum = [this.$t(`zero`), this.$t(`mon`), this.$t(`tue`), this.$t(`wed`), this.$t(`thu`), this.$t(`fri`), this.$t(`sat`), this.$t(`seven`), this.$t(`eight`), this.$t(`nine`)];
				let s = '';
				n = '' + n; // 数字转为字符串
