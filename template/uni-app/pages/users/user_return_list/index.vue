	...
	<view :style="colorStyle">
		...
		<view class='return-list' v-if="orderList.length">
			<view class='goodWrapper' v-for="(item,index) in orderList" :key="index"
				...
				<view class='orderNum'>{{$t(`order_number`)}}：{{item.order_id}}</view>
				<view class='item acea-row row-between-wrapper' v-for="(items,index) in item.cart_info" :key="index">
					...
					<view class='text'>
						...
						<view class='money'>
							{{$t(`money`)}}{{items.productInfo.attrInfo?items.productInfo.attrInfo.price:items.productInfo.price}}</view>
					</view>
				</view>
				<view class='totalSum'>{{$t(`total_of`)}} {{item.refund_num || 0}} {{$t(`items_total`)}} <text
						class='font-color price'>{{$t(`money`)}}{{item.refund_price}}</text></view>
			</view>
		</view>
		...
		<view v-if="orderList.length == 0  && !loading">
			<emptyPage :title="$t(`no_refund_order`)"></emptyPage>
		</view>
		<!-- #ifdef MP -->
