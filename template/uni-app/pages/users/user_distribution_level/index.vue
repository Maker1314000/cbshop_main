								...
								<template>
									<view class="level-grow-wrap">
										<view class="level-info" :class="{'lock-sty':item.grade > (levelInfo.grade || 0)}">
											<view class="level-info-title">{{$t(`level_1_rises`)}}</view>
											<view class="num">{{item.one_brokerage}}
												<text class="percent">%</text>
											</view>
										</view>
										<view class="level-info" :class="{'lock-sty':item.grade > (levelInfo.grade || 0)}">
											<view class="level-info-title">{{$t(`level_2_rises`)}}</view>
					...
					<view class="item acea-row row-middle" v-for="(item,index) in task" :key='item.id'>
						<view class="text">
							<view class="title">
								...
								<text class="mark">{{item.finish?$t(`completed`):$t(`undone`)}}</text>
							</view>
							<view class="process">
								<view
