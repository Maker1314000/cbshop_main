			...
			<view class='wrapper'>
				<view :hidden='currentTab != 0' class='list'>
					<form @submit="subCash">
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`cardholder`)}}</view>
							<view class='input'><input :placeholder='$t(`enter_cardholder`)' placeholder-class='placeholder'
									name="name"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`card_number`)}}</view>
							<view class='input'><input type='number' :placeholder='$t(`enter_card_num`)' placeholder-class='placeholder'
									name="cardnum"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`bank`)}}</view>
							<view class='input'>
						...
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`withdraw`)}}</view>
							<view class='input'><input :placeholder='$t(`min_amount`)+minPrice' placeholder-class='placeholder'
									name="money" type='digit'></input></view>
						</view>
						<view class='tip'>
							{{$t(`current_amount`)}}: <text
								class="price">{{$t(`money`)}}{{userInfo.commissionCount}},</text>{{$t(`freeze_commission`)}}：{{$t(`money`)}}{{userInfo.broken_commission}}
						</view>
						<view class='tip'>
							{{$t(`note`)}}{{userInfo.broken_day}}{{$t(`day`)}}，{{$t(`withdrawn`)}}
						</view>
						<button formType="submit" class='bnt bg-color'>{{$t(`withdraw`)}}</button>
					</form>
				</view>
				<view :hidden='currentTab != 1' class='list'>
					<form @submit="subCash">
						<view class='item acea-row row-between-wrapper' v-if="!brokerageType">
							<view class='name'>{{$t(`account_num`)}}</view>
							<view class='input'><input :placeholder='$t(`fill_wechat`)' placeholder-class='placeholder'
									name="name"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`withdraw`)}}</view>
						...
						<view class='item acea-row row-top row-between' v-if="!brokerageType">
							<view class='name'>{{$t(`receipt_code`)}}</view>
				...
				<view :hidden='currentTab != 2' class='list'>
					<form @submit="subCash">
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`account_num`)}}</view>
							<view class='input'><input :placeholder='$t(`fill_alipay`)' placeholder-class='placeholder'
									name="name"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`withdraw`)}}</view>
						...
						<view class='tip'>
							{{$t(`note`)}}{{userInfo.broken_day}}{{$t(`money`)}}，{{$t(`withdrawn`)}}
						</view>
						<button formType="submit" class='bnt bg-color'>{{$t(`withdraw`)}}</button>
					</form>
				</view>
		...
		methods: {
			...
			getUserExtractBank: function() {
				...
				extractBank().then(res => {
					...
					array.unshift(this.$t(`select_bank`));
			...
			subCash(e) {
				...
				if (that.currentTab == 0) { //银行卡
					if (!value.name.trim()) return this.$util.Tips({
						title: this.$t(`fill_cardholder_name`)
					});
					if (!value.cardnum.trim()) return this.$util.Tips({
						title: this.$t(`fill_card_number`)
					});
					if (that.index == 0) return this.$util.Tips({
						title: this.$t(`select_bank`)
					});
