<template>
	<view v-if="status == -1 && !inloading" :style="colorStyle">
					...
					<view class='list'>
						<view class="item">
							<view class="acea-row row-middle">
								...
								<text class="item-name">{{$t(`agency_name`)}}</text>
								<input type="text" maxlength="30" :placeholder="$t(`enter_agent_name`)"
						...
						<view class="item">
							<view class="acea-row row-middle">
								...
								<text class="item-name">{{$t(`enter_name`)}}</text>
								<input type="text" :placeholder="$t(`enter_name`)" v-model="merchantData.name" @input="validateBtn"
									placeholder-class='placeholder' />
							</view>
						</view>
						<view class="item">
							<view class="acea-row row-middle">
								...
								<text class="item-name">{{$t(`contact_no`)}}</text>
								<input type="text" :placeholder="$t(`enter_contact_number`)" v-model="merchantData.phone"
						...
						<view class="item rel">
							<view class="acea-row row-middle">
								...
								<text class="item-name">{{$t(`code`)}}</text>
								<input type="text" :placeholder="$t(`enter_code`)" v-model="merchantData.code" @input="validateBtn"
						...
						<view class="item">
							<view class="acea-row row-middle">
								<!-- <i class="icon iconfont icon-shoujihao"></i> -->
								<text class="item-name">{{$t(`invitation_code`)}}</text>
								<input type="text" :placeholder="$t(`enter_agent_code`)" v-model="merchantData.division_invite"
									@input="validateBtn" placeholder-class='placeholder' />
							</view>
						</view>
						<view class="item no-border">
							<view class='acea-row row-middle'>
								<text class="item-title">{{$t(`upload_quali_pic`)}}</text>
								<text class="item-desc">({{$t(`up_to_10`)}})</text>
								<view class="upload">
						...
						<view class="item no-border">
							<checkbox-group @change='ChangeIsAgree'>
								<checkbox class="checkbox" :checked="isAgree ? true : false" />{{$t(`read_agree`)}}
							</checkbox-group>
							<button class="settleAgree" @click="getAgentAgreement">《{{$t(`agency_agreement`)}}》</button>
						</view>
						<button class='submitBtn' :class="isAgree === true ? 'on':''" @click="formSubmit">{{$t(`submit_application`)}}</button>
		...
		<view class="settlementAgreement" v-if="showProtocol">
			<view class="setAgCount">
				...
				<div class="title">{{$t(`agent_agreement`)}}</div>
				<view class="content">
	...
	<view class="settledSuccessMain" v-else-if='status == 0'>
		<view class="settledSuccessful">
			<image class="image" src="../static/success.png" alt="">
				<view class="title">{{$t(`info_success`)}}</view>
	...
	<view class="settledSuccessMain" v-else-if='status == 1'>
		<view class="settledSuccessful">
			<image class="image" src="../static/success.png" alt="">
				<view class="title">{{$t(`Approved`)}}</view>
				<view class="goHome" hover-class="none" @click="goHome">
					{{$t(`back_to_home`)}}
				</view>
		</view>
	...
	<view class="settledSuccessMain" v-else-if='status == 2'>
		<view class="settledSuccessful">
			<image class="image" src="../static/error.png" alt="">
				<view class="title">{{$t(`apply_not_approved`)}}</view>
		...
		methods: {
			...
			validateForm: function() {
				...
				if (!value.agent_name) return that.$util.Tips({
					title: that.$t(`enter_agent_name`)
				});
				if (!value.name) return that.$util.Tips({
					title: that.$t(`type_name`)
				});
				...
				if (that.isShowCode && !that.codeVal) return that.$util.Tips({
					title: that.$t(`fill_image_code`)
				});
				if (!that.images.length) return that.$util.Tips({
					title: that.$t(`upload_business`)
				});
				if (!that.isAgree) return that.$util.Tips({
					title: that.$t(`tick_agreement`)
				});
				that.validate = true;
				return true;
			},
