				...
				<view class="item-box">
					<view class='item' v-for="(item,index) in bastList" :key="index" @click="goDetail(item)">
						...
						<view class='text'>
							...
							<view class="type">
								<view class="type-sty" v-if="item.activity && item.activity.type == '1'">{{$t(`seckill`)}}</view>
								<view class="type-sty" v-if="item.activity && item.activity.type == '2'">{{$t(`haggle`)}}</view>
								<view class="type-sty" v-if="item.activity && item.activity.type == '3'">{{$t(`group`)}}</view>
							</view>
			...
			<block v-if="isIframe && !bastList.length">
				<view class='index-wrapper' v-if="isIframe && !fastList.length">
					...
					<view class='scroll-product'>
						<view class="empty-img">{{$t(`recommend_no_data`)}}</view>
					</view>
				</view>
			</block>
		</view>
