<template>
	<view>
		<view class="default" v-if="isIframe && !spikeList.length">
			<text>{{$t(`seckill_no_data`)}}</text>
		</view>
		<view class="seckill" v-if="spikeList.length && isShow && !isIframe">
			<view class="title acea-row row-between-wrapper">
				<view class="acea-row row-middle">
					<view class="name">{{$t(`limite_seckill`)}}</view>
					<view class="point">{{point}} {{$t(`field`)}}</view>
					<countDown :is-day="false" :tip-text="' '" :day-text="' '" :hour-text="' : '" :minute-text="' : '" :second-text="' '"
					 :datatime="datatime"></countDown>
				</view>
				<navigator url="/pages/activity/goods_seckill/index" hover-class="none" class="more acea-row row-center-wrapper">{{$t(`more`)}}<text class="iconfont icon-jiantou"></text></navigator>
			</view>
			<view class="conter">
				<scroll-view scroll-x="true" style="white-space: nowrap; vertical-align: middle;" show-scrollbar="false">
					<view class="itemCon" v-for="(item, index) in spikeList" :key="index" @click="goDetail(item)">
						<view class="item">
							...
							<view class="money">
								<view class="x_money">{{$t(`money`)}}<text class="num">{{item.price}}</text></view>
		...
		<view class="seckill" v-if="spikeList.length && isIframe">
			<view class="title acea-row row-between-wrapper">
					...
					<navigator class="more acea-row row-center-wrapper">{{$t(`more`)}}<text class="iconfont icon-jiantou"></text></navigator>
				</view>
				<view class="conter">
					<scroll-view scroll-x="true" style="white-space: nowrap; vertical-align: middle;" show-scrollbar="false">
						<view class="itemCon" v-for="(item, index) in spikeList" :key="index" @click="goDetail(item)">
