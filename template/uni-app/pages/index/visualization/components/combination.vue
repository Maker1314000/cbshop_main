<template>
	<view>
		<view class="default" v-if="isIframe && !combinationList.length">
			<text>{{$t(`group_no_data`)}}</text>
		</view>
		<view class="combination index-wrapper" v-if="combinationList.length&&isShow&&!isIframe">
			<view class='title acea-row row-between-wrapper'>
				<view class='text'>
					<view class='name line1'>{{$t(`group_benefits`)}}</view>
					<view class='line1'>{{$t(`open_group_price`)}}</view>
				</view>
						...
						<view class="item">
							...
							<view class="money">
								<view class="price acea-row row-middle">
									<view class="label">{{$t(`group_price`)}}</view>
									<view class="x_money">{{$t(`money`)}}<text class="num">{{item.price}}</text></view>
								</view>
								<view class="bnt">{{$t(`join_groupbuy`)}}</view>
							</view>
						</view>
					</view>
				</scroll-view>
