		...
		<view class="index-wrapper">
			<view class='wrapper' v-if="isShow && benefit.length">
				<view class='title acea-row row-between-wrapper'>
					...
					<view class='more' @click="gopage(titleInfo[2].val)">{{$t(`more`)}}<text class='iconfont icon-jiantou'></text>
					</view>
				</view>
			...
			<view class='wrapper' v-if="!isShow && isIframe && benefit.length">
				<view class='title acea-row row-between-wrapper'>
					...
					<view class='more' @click="gopage(titleInfo[1].val)">{{$t(`more`)}}<text class='iconfont icon-jiantou'></text>
					</view>
				</view>
				<promotionGood :benefit="benefit"></promotionGood>
			</view>
			<view class='wrapper' v-if="isIframe && !benefit.length">
				...
				<view class="empty-img">{{$t(`promotional_items`)}}</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		data() {
			return {
				benefit: [],
				salesInfo: this.$t(`in_stock_product`),
