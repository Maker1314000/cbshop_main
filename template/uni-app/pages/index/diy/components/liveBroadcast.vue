	...
	<view v-show="!isSortType">
		<view :style="[{'margin-top': mbConfig + 'rpx'},{'background':bg}]" v-if="liveList.length > 0" style="padding-bottom: 20rpx;">
			<view class="title-box">
				<text>{{$t(`recommend_goods`)}}</text>
				<navigator hover-class="none" url="/pages/columnGoods/live_list/index" class="more">{{$t(`more`)}}<text class="iconfont icon-jiantou"></text></navigator>
			</view>
			<!--  -->
							...
							<view class="goods-wrapper">
								<block v-if="item.goods.length <= 3">
									<view class="goods-item" v-for="(goods,index) in item.goods" :key="index">
										...
										<text class="line1">{{$t(`money`)}}{{goods.price}}</text>
									</view>
								</block>
								<block v-if=" item.goods.length > 3">
								...
								<block v-if="item.goods.length == 0">
									<view class="empty-goods" >{{$t(`no_product_see`)}}</view>
								</block>
							</view>
						</view>
					</navigator>
