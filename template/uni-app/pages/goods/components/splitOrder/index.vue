	...
	<view class='splitOrder' v-if="splitGoods.length">
		<view class="all" v-if="select_all">
			<checkbox-group @change="checkboxAllChange">
				...
				<text class='checkAll'>{{$t(`select_all`)}}</text>
			</checkbox-group>
		</view>
					...
					<view class='picTxt acea-row row-between-wrapper'>
						...
						<view class='text'>
							...
							<view class='infor line1'>
								{{$t(`attributes`)}}：{{item.cart_info.productInfo.attrInfo.suk || $t(`default`)}}</view>
							<view class='money'>{{$t(`money`)}}{{item.cart_info.truePrice}}</view>
						</view>
