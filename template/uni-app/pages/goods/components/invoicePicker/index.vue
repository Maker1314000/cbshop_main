<template>
	<view>
		...
		<view class="popup" :class="{ on: invShow }">
			<view class="popup-hd">{{$t(`head_select`)}}<text class="iconfont icon-guanbi" @click="invClose"></text></view>
			<scroll-view class="popup-bd" scroll-y="true">
								...
								<view class="acea-row row-middle">
									<view class="name-wrap acea-row row-middle">
										<view class="name-group">
											...
											<view v-if="item.is_default" class="default">{{$t(`default`)}}</view>
										</view>
									</view>
									<view class="type" :class="{special: item.type === 2}">
										{{item.header_type === 1 ? $t(`personal`) : $t(`enterprise`)}} {{item.type === 1 ? $t(`ordinary`) : $t(`dedicated`)}}
									</view>
								</view>
								<view class="acea-row row-bottom">
									<view class="info-wrap">
										<view class="email">{{$t(`email`)}} {{item.email}}</view>
										<view v-if="item.header_type === 1" class="tel">{{$t(`contact_no`)}} {{item.drawer_phone}}
										</view>
										<view v-else class="number">{{$t(`tax_num`)}}{{item.duty_number}}</view>
									</view>
									<navigator v-if="!isOrder" class="navigator"
										:url="`/pages/users/user_invoice_form/index?from=order_confirm&id=${item.id}&${urlQuery}`"
										hover-class="none"><text class="iconfont icon-bianji"></text>{{$t(`edit`)}}</navigator>
				...
				<view v-else class="empty">
					...
					<view>{{$t(`invoice_info`)}}~</view>
				</view>
			</scroll-view>
			<view class="popup-ft">
				<navigator v-if="!isOrder" class="navigator"
					:url="`/pages/users/user_invoice_form/index?from=order_confirm&${urlQuery}`" hover-class="none">
					<text class="iconfont icon-fapiao"></text>{{$t(`add_new_header`)}}</navigator>		
				<navigator v-else class="navigator"
					:url="`/pages/users/user_invoice_form/index?order_id=${orderId}&from=order_details&${urlQuery}`" hover-class="none">
					<text class="iconfont icon-fapiao"></text>{{$t(`add_new_header`)}}</navigator>
				<button class="button" plain @click="invCancel">{{$t(`no_invoice`)}}</button>
				<button v-if="isOrder" class="button" plain @click="invSub">{{$t(`confirm_sub`)}}</button>
			</view>
		</view>
	</view>
</template>
