	...
	<view>
		<view class="cartList" :class="cartData.iScart?'on':''">
			<view class="title acea-row row-between-wrapper">
				<view class="name">{{$t(`selected_items`)}}</view>
				<view class="del acea-row row-middle" @click="subDel"><view class="iconfont icon-shanchu1"></view>{{$t(`empty`)}}</view>
			</view>
			<view class="list">
					...
					<view class="txtPic">
						...
						<view v-if="item.attrStatus && item.status">
							...
							<view class="bottom acea-row row-between-wrapper">
								<view class="money">{{$t(`money`)}}<text class="num">{{item.truePrice}}</text></view>
						...
						<view class="noBnt" v-else-if="!item.attrStatus">{{$t(`sold_out`)}}</view>
						<view class="noBnt" v-else-if="!item.status">{{$t(`removed`)}}</view>
						<view class="delTxt acea-row row-right" v-if="!item.status || !item.attrStatus"><text @click="oneDel(item.id,index)">{{$t(`delete`)}}</text></view>
					</view>
				</view>
			</view>
		</view>
