	...
	<view>
		<view class='coupon-list-window' :class='coupon.coupon==true?"on":""'>
			<view v-if="coupon.count" class="nav acea-row row-around">
				<view v-if="coupon.count[2]" :class="['acea-row', 'row-middle', coupon.type === 2 ? 'on' : '']" @click="setType(2)">{{$t(`commodity_voucher`)}}</view>
				<view v-if="coupon.count[1]" :class="['acea-row', 'row-middle', coupon.type === 1 ? 'on' : '']" @click="setType(1)">{{$t(`category_coupons`)}}</view>
				<view v-if="coupon.count[0]" :class="['acea-row', 'row-middle', coupon.type === 0 ? 'on' : '']" @click="setType(0)">{{$t(`universal_coupon`)}}</view>
			</view>
			<view class='title' v-else>{{$t(`coupon`)}}<text class='iconfont icon-guanbi' @click='close'></text></view>
				  ...
				  <view class="moneyCon acea-row row-center-wrapper">
					...
					<view class='text'>
						<view class='condition line2' :class="coupon.count?'':'order'">
							<span class='line-title' :class='item.is_use && coupon.count?"gray":""' v-if='item.type===0'>{{$t(`universal_coupon`)}}</span>
							<span class='line-title' :class='item.is_use && coupon.count?"gray":""' v-else-if='item.type===1'>{{$t(`category_coupons`)}}</span>
							<span class='line-title' :class='item.is_use && coupon.count?"gray":""' v-else>{{$t(`commodity_voucher`)}}</span>
						...
						<view class='data acea-row row-between-wrapper'>
							...
							<view v-if="coupon.count">
								<view class='bnt gray' v-if="item.is_use">{{item.use_title || $t(`received`)}}</view>
								<view class='bnt bg-color' v-else>{{coupon.statusTile || $t(`get_it_now`)}}</view>
							</view>
