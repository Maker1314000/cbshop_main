<template>
	<view class="goodsList">
		<view class="item" v-for="(item,index) in tempArr" :key='index' @click="goDetail(item)">
			<view class="pictrue">
				<span class="pictrue_log pictrue_log_class" v-if="item.activity && item.activity.type === '1'">{{$t(`seckill`)}}</span>
				<span class="pictrue_log pictrue_log_class" v-if="item.activity && item.activity.type === '2'">{{$t(`haggle`)}}</span>
				<span class="pictrue_log pictrue_log_class" v-if="item.activity && item.activity.type === '3'">{{$t(`group`)}}</span>
			...
			<view class="bottom acea-row row-between-wrapper">
				<view class="sales acea-row row-middle">
					<view class="money font-color"><text>{{$t(`money`)}}</text>{{item.price}}</view>
					<view>{{$t(`sold`)}} {{item.sales}}</view>
				</view>
				...
				<view class="bnt end" v-else>{{$t(`sold_out`)}}</view>
			</view>
		</view>
	</view>
</template>
