		...
		<view class='coupon-window' :class='window==true?"on":""'>
			...
			<view class='couponWinList'>
				<view class='item acea-row row-between-wrapper' v-for="(item,index) in couponList" :key="index">
					<view class='money font-color'>{{$t(`money`)}}<text class='num'>{{item.coupon_price}}</text></view>
					<view class='text'>
						<view class='name'>{{$t(`full_of_shopping`)}}{{item.use_min_price}}{{$t(`reduce`)}}{{item.coupon_price}}</view>
						<view v-if="item.coupon_time">{{$t(`after_receiving`)}}{{item.coupon_time}}{{$t(`days_available`)}}</view>
						<view v-else>
							{{item.start_time ? item.start_time+'-' : ''}}{{item.end_time === 0 ? $t(`unlimited_time`) : item.end_time}}
						</view>
					</view>
				</view>
			</view>
			<view class='lid'>
				<navigator hover-class='none' url='/pages/users/user_get_coupon/index' class='bnt'>{{$t(`get_it_now`)}}</navigator>
