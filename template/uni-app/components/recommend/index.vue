<template>
	<view class='recommend' :style="colorStyle">
		<view class='title acea-row row-center-wrapper'>
			...
			<text class='name'>{{$t(`hot_recom`)}}</text>
		...
		<view class='recommendList acea-row row-between-wrapper'>
			<view class='item' v-for="(item,index) in hostProduct" :key="index" hover-class='none' @tap="goDetail(item)">
				<view class='pictrue'>
					...
					<span class="pictrue_log_big pictrue_log_class" v-if="item.activity && item.activity.type === '1'">{{$t(`seckill`)}}</span>
					<span class="pictrue_log_big pictrue_log_class" v-if="item.activity && item.activity.type === '2'">{{$t(`haggle`)}}</span>
					<span class="pictrue_log_big pictrue_log_class" v-if="item.activity && item.activity.type === '3'">{{$t(`group`)}}</span>
				</view>
				<view class='name line1'>{{item.store_name}}</view>
				<view class='money font-color'>{{$t(`money`)}}<text class='num'>{{item.price}}</text></view>
			</view>
		</view>
	</view>
</template>
