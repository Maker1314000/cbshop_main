	...
	<view :style="colorStyle">
		<view class="product-window"
				...
				<view class="text">
					...
					<view class="money font-color">
						<view class="acea-row row-middle">
							{{$t(`money`)}}<text class="num">{{ attr.productSelect.price }}</text>
							<text class='vip-money'
								v-if="is_vip>0 && attr.productSelect.vip_price">{{$t(`money`)}}{{attr.productSelect.vip_price}}</text>
							<view class="vipImg" v-if="is_vip>0 && attr.productSelect.vip_price">
								<image src="../../static/images/svip.gif"></image>
							</view>
						</view>
						<text class="stock" v-if='isShow'>{{$t(`stock`)}}: {{ attr.productSelect.stock }}</text>
						<text class='stock' v-if="limitNum">{{type ? $t(`stock`) : $t(`limited`)}}: {{attr.productSelect.quota}}</text>
					</view>
				</view>
			...
			<view class="joinBnt bg-color"
				v-if="iSbnt && attr.productSelect.product_stock>0 &&attr.productSelect.quota>0" @click="goCat">{{$t(`join_group`)}}
			</view>
			<view class="joinBnt on"
				v-else-if="(iSbnt && attr.productSelect.quota<=0)||(iSbnt &&attr.productSelect.product_stock<=0)">{{$t(`sold_out`)}}
			</view>
			<view class="joinBnt bg-color" v-if="iScart && attr.productSelect.stock" @click="goCat">{{$t(`confirm`)}}</view>
			<view class="joinBnt on" v-else-if="iScart && !attr.productSelect.stock">{{$t(`sold_out`)}}</view>
		</view>
